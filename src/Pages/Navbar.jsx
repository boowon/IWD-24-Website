    ...
    <div>
      ...
      <section id="mobile-menu" onClick={toggleMenu} className={`fixed z-[1000] top-0 left-0 lg:hidden w-full h-full bg-white ${isOpen ? 'flex' : 'hidden' } w-full origin-top animate-open-menu flex-col content-evenly items-center justify-center text-5xl`}>
        <nav className="flex absolute min-h-screen flex-col items-center py-8 content-evenly justify-center gap-4 bg-white" aria-label="mobile">
          <a href="#home" onClick={toggleMenu} className={activeSection === 'home' ? 'text-green hover:text-light-green' : 'text-blue hover:text-light-blue'}>Home</a>
          <hr className='h-1 w-[60vw] border-gray-500'/>
          <a href="#about" onClick={toggleMenu} className={activeSection === 'about' ? 'text-green hover:text-light-green' : 'text-blue hover:text-light-blue'}>About</a>        
          <hr className='h-1 w-[60vw] border-gray-500'/>
          <a href="#faq" onClick={toggleMenu} className={activeSection === 'faq' ? 'text-green hover:text-light-green' : 'text-blue hover:text-light-blue'}>FAQ</a>
          <hr className='h-1 w-[60vw] border-gray-500'/>
          <a href="#quiz" onClick={toggleMenu} className={activeSection === 'quiz' ? 'text-green hover:text-light-green' : 'text-blue hover:text-light-blue'}>Quiz</a>
          <hr className='h-1 w-[60vw] border-gray-500'/>
          <a href="#register" onClick={toggleMenu} className={activeSection === 'register' ? 'text-green hover:text-light-green' : 'text-blue hover:text-light-blue'}>Register</a>      
        </nav>
      </section>
      <nav className="fixed bg-opacity-30 bg-white  w-[100%] top-0 z-50 font-montserrat text-2xl font-medium hidden lg:block">
        <div className=" pt-5 px-4 lg:pb-2 lg:pt-3 sm:px-6 lg:px-8">
